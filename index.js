...
const gamesSum = GAMES_JSON.reduce ( (acc, game) => {return acc + game.pledged;}, 0);
const totalGames = GAMES_JSON.length;
const numUnfundedGames = (GAMES_JSON.filter ((game) => {return game.pledged < game.goal;})).length

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${gamesSum.toLocaleString()} has been raised for ${totalGames} games.
...
firstGameContainer.append(topPledgeName);

// do the same for the runner up item
const runnerUpName = document.createElement("p");
runnerUpName.innerHTML = second.name;
secondGameContainer.append(runnerUpName);
